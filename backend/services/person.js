...
serviceRouter.delete("/person/:id", function (request, response) {
  ...
  try {
    ...
    console.log(
      "Service Person: Deletion of record successfull, id=" + request.params.id,
    );
    response.status(200).json({ gelöscht: true, eintrag: obj });
  } catch (ex) {
    console.error(
      "Service Person: Error deleting record. Exception occured: " + ex.message,
    );
    response.status(400).json({ fehler: true, nachricht: ex.message });
  }
});
